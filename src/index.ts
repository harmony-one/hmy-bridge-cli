...
import { IHmyClient } from './blockchain';
...
export class BridgeSDK {
  ...
  sendToken = async (
    ...
    const web3Client = getWeb3Client({ 
      ...this.initParams.ethClient, 
      // explorerURL: configs.networks[params.network].blockExplorerUrls[0],
      nodeURL: configs.networks[params.network].rpcUrls[0]
    });

    if (this.useMetamask) {
      web3Client.setUseMetamask(this.useMetamask);
    }

    let account = '';

    if (this.privateKey) {
      account = await web3Client.addWallet(this.privateKey);
    }
    ...
    return await operation(
      {
        ...params,
        oneAddress: params.oneAddress || account,
        ethAddress: params.ethAddress || account,
